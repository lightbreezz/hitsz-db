...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    off_t ret = lseek(fd, static_cast<off_t>(page_no) * PAGE_SIZE, SEEK_SET);
    if (ret == (off_t)-1) {
        throw UnixError();
    }
...
void DiskManager::destroy_file(const std::string &path) {
    ...
    // 调用unlink()函数
    // 注意不能删除未关闭的文件
    // Cannot destroy if file is opened
...
int DiskManager::open_file(const std::string &path) {
    ...
    page_id_t pages = 0;
    if (file_size > 0) pages = static_cast<page_id_t>(file_size / PAGE_SIZE);
    fd2pageno_[fd] = pages;
    return fd;
}
