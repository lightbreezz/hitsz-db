...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid& rid, Context* context) const {
    ...
    // context is not used
    (void)context;
    // 1. 获取指定记录所在的page handle(include bitmap和page_hdr等信息)
    RmPageHandle page_handle = fetch_page_handle(rid.page_no);
    if (!Bitmap::is_set(page_handle.bitmap, rid.slot_no)) {
        // note: 获取其他类的实例(这里调用buffer_pool_manager)
    ...
    // get the record so unpin the page
    buffer_pool_manager_->unpin_page(page_handle.page->get_page_id(), false);
    return rec;
}
...
void RmFileHandle::insert_record(const Rid& rid, char* buf) {
    // hard: 换了好几种逻辑，感觉本质上就是维护链表
    RmPageHandle page_handle = fetch_page_handle(rid.page_no);
    if (Bitmap::is_set(page_handle.bitmap, rid.slot_no)) {
        buffer_pool_manager_->unpin_page(page_handle.page->get_page_id(), false);
        throw InternalError("RmFileHandle::insert_record slot already exists");
    }
    ...
    if (page_handle.page_hdr->num_records == file_hdr_.num_records_per_page) {
        ...
        if (file_hdr_.first_free_page_no == page_no) {
            ...
            while (prev_no != RM_NO_PAGE) {
                ...
                if (prev_handle.page_hdr->next_free_page_no == page_no) {
